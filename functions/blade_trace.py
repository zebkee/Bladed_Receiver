...
def generic_traceMP(index,rec_obj,surf_ls,crit_ls,absorp,rays_per_bund=10000,tot_rays=100000,bins=50,render_bool=True,procs=1):
	"""Raytraces a generic receiver, where rec_obj is an assembled object, surf_ls is a list of all surfaces and
	   crit_ls is a list of all surfaces that are to be plotted in the histogram"""
	heliocsv = "/home/zebedee/Tracer/Tracer/examples/sandia_hstat_coordinates.csv"
	start = timeit.default_timer()
	...
	scene.traceMP(procs,rph=rph,render = render_bool)
	H1, boundlist, extent, area = scene.hist_comb(no_of_bins=bins)
	energy,hits = scene.energies(A)
	print(energy)
	print(hits)
	...
	extent1 = extent
	boundlist1 = boundlist # First assignment is enough
	rays_used += rph*218*procs #multiply by number of processors
	del scene #Try to free up memory
	...
	while rays_used < tot_rays:
		...
		scene.gen_plant()
		scene.aim_field()
		scene.traceMP(procs,rph=rph,render=False)
		rays_used += rph*218*procs #ditto
		H1, boundlist, extent, area = scene.hist_comb(no_of_bins=bins)
		energy, hits = scene.energies(A)
		print(energy)
		print(hits)
...
def single_traceMP(index,rec_obj,surf_ls,crit_ls,absorp,rays_per_bund=20000,bins=50,render_bool=False,procs=1):
	"Does a single trace using multiple processsors to test validity of results"""
	heliocsv = "/home/zebedee/Tracer/Tracer/examples/sandia_hstat_coordinates.csv"
	start = timeit.default_timer()
