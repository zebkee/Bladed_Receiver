...
def z_hist1d(H_norm,binny,show=True):
   	"""Creates a 1d histogram to disply flux uniformity"""
	bigarray = [item for sublist in H_norm for item in sublist]
	maxconc = max(bigarray)
	print("The maximum concentration is "+str(maxconc)+" kW per square metre")
...
class TowerScene():
    ...
    def trace(self, hstat_rays = 10, iters = 1000, minE = 1e-5, render=True):
    ...
    def traceMP(self, procs = 3, hstat_rays = 10, iters = 1000, minE = 1e-5, render = True):
	"""Traces using multiple processors. May have to reduce hstatrays by factor of procs if memory runs out. Do the division of total rays beforehand."""
	# e= None just prevents the buildup of memory
	e = None
	self.hstat_rays = hstat_rays
        sun_vec = solar_vector(self.sun_az*degree, self.sun_elev*degree)
	n = 1
